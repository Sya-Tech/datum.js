(function (global, factory) {
   ...
   function getVueText(node) {
       var vText = node.getAttribute('v-text');
       return vText ? babelHelpers.defineProperty({}, vText, node.innerText) : null;
   }

   function getVueFor(node) {
       var expression = node.getAttribute('v-for');
       var inMatch = expression ? expression.match(/(.*) (?:in|of) (.*)/) : null;
       return inMatch ? { vueFor: babelHelpers.defineProperty({}, inMatch[2].trim(), []), name: inMatch[1].trim(), key: inMatch[2].trim() } : { vueFor: null, name: null, key: null };
   }

   function DOMTreeWalker(DOMNode) {
       var vueData = arguments.length <= 1 || arguments[1] === undefined ? {} : arguments[1];
       var scope = arguments.length <= 2 || arguments[2] === undefined ? [] : arguments[2];

       var vueText = getVueText(DOMNode);

       var _getVueFor = getVueFor(DOMNode);

       var vueFor = _getVueFor.vueFor;
       var name = _getVueFor.name;
       var key = _getVueFor.key;

       scope = key ? scope.concat(key) : scope;
       for (var i = 0; i < DOMNode.children.length; i++) {
           var node = DOMNode.children[i];
           DOMTreeWalker(node, vueData, scope);
       }
       if (scope.length) {
           vueData[scope[0]] = vueData[scope[0]] || [];
           var scopeData = scope.length ? vueData[scope[0]] : vueData;
           console.log(vueText);
